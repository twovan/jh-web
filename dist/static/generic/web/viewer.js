...
(function mozPrintCallbackPolyfillClosure() {
  ...
  window.addEventListener('keydown', newKeydown, true);
  if (hasAttachEvent) {
  ...
  if ('onbeforeprint' in window) {
    ...
    stopPropagationIfNeeded = function (event) {
...
var PDFHistory = {
  ...
  initialize: function pdfHistoryInitialize(fingerprint, linkService) {
    ...
    var self = this;
    pdfHistoryPopstate = function (evt) {
      evt.preventDefault();
      evt.stopPropagation();
    ...
    window.addEventListener('popstate', pdfHistoryPopstate, false);

    pdfHistoryBeforeUnload = function () {
...
var PDFViewer = (function pdfViewer() {
  ...
  function PDFViewer(options) {
    this.container = options.container;
    if (!this.container && !options.viewer) {
      return false
    }
    this.viewer = options.viewer || options.container.firstElementChild;
  ...
  download: function pdfViewDownload() {
    ...
    this.pdfDocument.getData().then(
      function getDataSuccess(data) {
        ...
        downloadManager.download(blob, url, window.parent.jzRoot + '.pdf');
      },
...
presentationmodechangedFC = function (e) {
...
window.addEventListener('presentationmodechanged', presentationmodechangedFC);
...
window.addEventListener('updateviewarea', updateviewareaFC, true);

webViewerResizeFC = function (evt) {
  if (PDFViewerApplication.initialized &&
...
window.addEventListener('resize', webViewerResizeFC);

webViewerHashchangeFC = function (evt) {
  if (PDFHistory.isHashChangeUnlocked) {
    PDFViewerApplication.setHash(document.location.hash.substring(1));
  }
}

window.addEventListener('hashchange', webViewerHashchangeFC);
...
window.addEventListener('localized', localizedFC, true);

scalechangeFC = function (evt) {
...
window.addEventListener('scalechange', scalechangeFC, true);

pagechangeFC = function (evt) {
...
window.addEventListener('pagechange', pagechangeFC, true);
